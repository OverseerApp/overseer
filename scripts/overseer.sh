...
install_dotnet_dependencies() {
    ...
    if [ ! -d "$dotnetPath" ]; then
        log ".NET is not installed. Installing .NET..."
        ...
        log "Installing Emgu.CV dependencies..."
        apt-get install -y \
            libopencv-dev \
            libgdiplus \
            libc6-dev
...
stop_overseer() {
    local overseerPID=$(ps auxf | grep ${OVERSEER_EXECUTABLE} | grep -v grep | awk '{print $2}')
    
    if [ -f "$SERVICE_PATH" ] && systemctl is-active --quiet $SERVICE_NAME 2>/dev/null; then
        log "Stopping Overseer service..."
        systemctl stop $SERVICE_NAME
        
        # Wait for the service to fully stop
        local max_wait=30
        local waited=0
        while systemctl is-active --quiet $SERVICE_NAME && [ $waited -lt $max_wait ]; do
            sleep 1
            waited=$((waited + 1))
        done
    ...
    # Also check for any orphaned process
    if [ -n "${overseerPID}" ]; then
        log "Killing process: $overseerPID"
        kill ${overseerPID} 2>/dev/null || true
        sleep 2
    fi
...
create_service_file() {
    ...
    log "Service file created and enabled"
}
...
do_install() {
    ...
    update_system_packages
    install_dotnet_dependencies "$dotnetPath"
...
do_update() {
    ...
    if ! stop_overseer; then
        log_error "Failed to stop Overseer"
        exit 1
    fi
